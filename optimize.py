...
def get_params_part_7(x):
  ...
  fixed_params = (
    ...
    ball['location'][0], ball['location'][1], 0, ball["radius"], ball["radius"], 1, ball["linear_velocity"][0], ball["linear_velocity"][1],
    destination['x'], destination['y'] - 1/3 * destination['height'], 0, destination['width'] / 2, 1 / 3 * destination['height'] / 2, 0, 0, 0,
    destination['x'] + 0.4 * destination['width'] , destination['y'], 0, 0.2 * destination['width'] / 2, destination['height'] / 2, 0, 0, 0,
    destination['x'] - 0.4 * destination['width'] , destination['y'], 0, 0.2 * destination['width'] / 2, destination['height'] / 2, 0, 0, 0,
  )
  obstacles = env_configs['obstacles']
  optimized_params = np.array([(x[3*i], x[3*i+1], x[3*i+2], o["width"] / 2, o["height"] / 2, 0, 0, 0)  for i, o in enumerate(obstacles)]).ravel()
